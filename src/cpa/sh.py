...
class SH(CPA):
    ...
    def hop_prob(self):
        ...
        # Calculate hopping probabilities (vectorized)
        mask = np.arange(self.mol.nst) != self.rstate
        self.prob = np.where(mask,
            -2. * self.mol.rho.real[:, self.rstate] * self.mol.nacme[:, self.rstate] * self.dt
            / self.mol.rho.real[self.rstate, self.rstate], 0.)
